...
// Placeholder Chart Component (Dibuat lebih ringkas)
const PlaceholderStat = ({ title, value, colorClass }) => (
    <div className={`p-3 rounded-lg ${colorClass} flex flex-col justify-center items-start border border-gray-700/50`}>
...
const AssetDetailPanel = ({ asset, onClose }) => {
    ...
    if (!asset) {
        return (
            <div className="sticky top-4 h-[calc(100vh-5rem)] bg-gray-800 border border-gray-700 rounded-lg p-6 flex flex-col items-center justify-center text-center shadow-lg">
                ...
                <p className="text-gray-400 text-sm mt-1">View details and analytics here.</p>
            </div>
        );
    }
    ...
    return (
        ...
        <div className="sticky top-4 h-[calc(100vh-5rem)] bg-gray-800 border border-gray-700 rounded-lg p-5 flex flex-col overflow-hidden shadow-lg">
            
            {/* Header: Title dan Close Button */}
            <div className="flex justify-between items-start pb-3 mb-4 border-b border-gray-700">
                <div>
                    <h1 className="text-lg font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500 line-clamp-1">
                        {asset.title} 
                    </h1>
                    <span className="text-xs font-semibold text-purple-400 bg-purple-900/30 px-2 py-0.5 rounded mt-1 inline-block">SELECTED</span>
                </div>
            ...
            <div className="flex-grow space-y-5 overflow-y-auto pr-2 custom-scrollbar">
                ...
                {/* Compact Top Section: Image & Short Description */}
                <div className="flex flex-col sm:flex-row gap-4">
                    {/* Image (Dibuat lebih kecil) */}
                    <div className="w-full sm:w-1/3 h-36 bg-gray-700 rounded-md overflow-hidden flex items-center justify-center border border-gray-600 flex-shrink-0">
                        {asset.mediaUrl ? (
                    ...
                    <div className='flex-grow'>
                        <p className="text-sm text-gray-300 font-light mb-2">Description</p>
                        <p className="text-xs text-gray-400 line-clamp-5 max-h-24 overflow-hidden">
                            {asset.description || 'No description available.'}
                        </p>
                    </div>
                </div>

                {/* Stat Grid (Lebih Elegan) */}
                <div className='grid grid-cols-2 gap-3 pt-3 border-t border-gray-700/50'>
                    <PlaceholderStat title="Estimated Views (30D)" value="24K" colorClass="bg-pink-900/10 text-pink-300" />
                    <PlaceholderStat title="License Minted (Total)" value="8K" colorClass="bg-cyan-900/10 text-cyan-300" />
                </div>
