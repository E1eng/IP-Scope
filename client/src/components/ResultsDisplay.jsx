...
const ErrorDisplay = ({ message }) => (
  <div className="col-span-full flex flex-col items-center justify-center p-16 text-center text-red-300 bg-red-900/40 rounded-lg border border-red-700 shadow-xl">
    ...
    <p className="text-sm mt-4 text-gray-400">Ensure the API Key in server/.env is valid and the backend server is running.</p>
  </div>
);
...
const EmptyDisplay = () => (
    ...
    <p className="text-lg mt-2">Try searching with different keywords or adjusting filters.</p>
  </div>
);
...
const ResultsDisplay = ({ isLoading, error, results, hasSearched, onAssetClick, selectedAssetId }) => {
  const renderContent = () => {
    if (isLoading) {
      return Array.from({ length: 12 }).map((_, index) => <SkeletonCard key={index} />);
    }
    ...
    return results.map((asset, index) => (
      <AssetCard 
        ...
        isSelected={selectedAssetId === asset.ipId}
      />
    ));
  };
  ...
  if (!hasSearched && !isLoading) {
    return (
        <div className="text-center text-gray-500 p-16 bg-gray-800/40 rounded-lg border border-gray-700 shadow-md">
            ...
            <p className="text-lg font-light">Enter a query to load and explore intellectual property assets.</p>
        </div>
    );
  }

  // ▼▼▼ PERBAIKAN: Mengurangi jumlah kolom di layar XL ▼▼▼
  return (
